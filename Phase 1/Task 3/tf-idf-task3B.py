...
# Creates the dictionary with query Id and queries
def create_stem_query_dict(location):
    data = open(location, 'r')
    count = 1
    for line in data:
        queries_dict[str(count)] = line.strip()  # extract query text and remove extra spaces and blank lines
        count += 1
...
def write_to_file_stop(scores, query_id, path):

    path = path + "TFIDF_Score_With_StopWords_For_Query_" + query_id + ".txt"
    #path = path + "TFIDFScore_With_Stemming_For_Query_" + query_id + ".txt"
    ...
    for doc_id, score in scores:
        ...
        if count > 100:
            break
        output_file.write("%s Q0 %s %s %s TFIDF_Model_With_StopWords_CS6200\n" % (query_id, doc_id, count, score))
        count += 1


# To write the list to file
def write_to_file_stem(scores, query_id, path):
...
def tf_idf_score(output_file,is_stop_model):
    for query_id, query in queries_dict.items():
        ...
        print(scores_sorted)
        if is_stop_model == True:
            # print top 100 scores to the file
            write_to_file_stop(scores_sorted, query_id, output_file)
        else:
            #print top 100 scores to the file
            write_to_file_stem(scores_sorted, query_id, output_file)
...
def main():
    ...
    cacm_stem_queries_location = "D:\\master study\\courses plan\\2018 Fall\\cs6200 Information retrieval\\project\\test-collection\\test-collection\\cacm_stem.query.txt"
    # location for unigrams stem inverted indexes with term frequency
    inverted_stem_index_location = "D:\\master study\\courses plan\\2018 Fall\\cs6200 Information retrieval\\project\\stemmed_index_and_corpus\\inverted_lists_stemmed\\inverted_list_term_freqency.txt"

    output_file_B = "C:\\Users\\limin\\PycharmProjects\\IR-Project\\phase1\\Task3\\outputB\\"

    #task B: stem
    create_stem_query_dict(cacm_stem_queries_location)
    create_inverted_index_dict(inverted_stem_index_location)
    tf_idf_score(output_file_B,False)
