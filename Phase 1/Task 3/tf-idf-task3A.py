...
def create_stop_list(location):
    #global stop_list
    #stop_list = []
    with open(location, 'r') as myfile:
        raw_stop_list = myfile.read().strip("\n").split("\n")
    for item in raw_stop_list:
        stop_list.append(item.strip())
...
def main():
    ...
    common_words_location = "D:\\master study\\courses plan\\2018 Fall\\cs6200 Information retrieval\\project\\test-collection\\test-collection\\common_words"
    # outfile file location
    output_file_A = "C:\\Users\\limin\\PycharmProjects\\IR-Project\\phase1\\Task3\\outputA\\"

    #task A: stop
    create_query_dict(cacm_queries_location)
    create_inverted_index_dict(inverted_index_location)
    create_stop_list(common_words_location)
    #print(stop_list)
    tf_idf_score(output_file_A,True)
